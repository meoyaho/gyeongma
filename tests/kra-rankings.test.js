...
test('reads the first seven unique Korean horse names from the KRA ranking table', () => {
  const rows = ['로쉬', '라온사일런스', '서클에이', '로열삭스', '글로벌챔프', '라온플로렌스', '제너럴윈드', '한센브레이싱']
    ...
    .join('');
  assert.deepEqual(parseKraRankingHtml(`<table>${rows}</table>`), ['로쉬', '라온사일런스', '서클에이', '로열삭스', '글로벌챔프', '라온플로렌스', '제너럴윈드']);
});
