...
const voice = new VoiceController({
  onCalls: count => {
    ...
    else if (room?.phase === 'lobby') { localCalls += count; $('mic-transcript').textContent = `“${activeName}” ${localCalls}회 인식`; }
  },
  ...
  onTranscript: text => {
    ...
    const count = localCalls ? `이름 ${localCalls}회 인식` : '';
    $('mic-transcript').textContent = text ? `들린 말: ${text}${count ? ` · ${count}` : ''}` : count;
  },
